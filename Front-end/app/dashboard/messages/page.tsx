...
export default function MessagesPage() {
  ...
  async function startNew() {
    const id = parseInt(newPartnerId);
    if (!id) return;
    const partner = { partner_id: id, partner_name: `مستخدم #${id}`, last_message: "", last_at: new Date().toISOString(), unread_count: 0 };
    setActivePartner(partner);
    activePartnerRef.current = partner;
    setMessages([]);
    setNewPartnerId("");
      ...
      <div className="relative min-h-full" dir="rtl">
        ...
        >
          ...
          <div className="w-64 shrink-0 flex flex-col bg-white border border-gray-100 rounded-[1.75rem] shadow-sm overflow-hidden">
            {/* Header */}
            <div className="px-4 py-4 border-b border-gray-100 bg-slate-50/50">
              <p className="text-slate-900 font-extrabold text-sm mb-3">المحادثات</p>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-xl text-sm text-slate-900 placeholder-gray-400 border border-gray-200 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/10 transition-all"
                  onKeyDown={(e) => e.key === "Enter" && startNew()}
                />
                <button
                  onClick={startNew}
                  className="w-9 h-9 rounded-xl flex items-center justify-center bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 transition-all"
                >
            ...
            {!activePartner ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-4 text-slate-400">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">أو ابدأ محادثة جديدة بإدخال رقم المستخدم</p>
                </div>
              </div>
            ) : (
              <>
